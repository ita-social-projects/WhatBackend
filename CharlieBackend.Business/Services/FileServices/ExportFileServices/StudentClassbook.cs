        ...
        public async Task FillFileAlternate(StudentsClassbookResultDto data)
        {
            if (data.StudentsPresences != null)
            {
                ...
                for (int row = 0; row < data.StudentsPresences.Count(); row++)
                {
                    ...
                    xlRow.Cell(4).Value = presence.LessonId;
                    xlRow.Cell(5).Value = presence.LessonDate;
                    xlRow.Cell(6).Value = presence.Presence;
                }
            }
            ...
            if (data.StudentsMarks != null)
            {
                ...
                for (int row = 0; row < data.StudentsMarks.Count(); row++)
                {
                    ...
                    xlRow.Cell(4).Value = mark.LessonId;
                    xlRow.Cell(5).Value = mark.LessonDate;
                    xlRow.Cell(6).Value = mark.StudentMark;
                    xlRow.Cell(7).Value = mark.Comment;
                }
            }
        }
    }
