using CharlieBackend.Core.DTO.Schedule;
using CharlieBackend.Core.Entities;
using System;
using System.Collections.Generic;
using System.Text;

namespace CharlieBackend.Business.Services.ScheduleServiceFolder
{
    class ScheduledEventHandlerWeekly : ScheduledEventHandlerBase
    {
        public ScheduledEventHandlerWeekly(EventOccurence source, ContextForCreateScheduleDTO context, PatternForCreateScheduleDTO pattern) 
            : base(source, context, pattern)
        {
            _index = 7;
        }

        public override DateTime GetStartDate(int index)
        {
            DateTime result = _source.EventStart;
            
            return result.AddDays(result.DayOfWeek <= _pattern.DaysOfWeek[index]
                    ? _pattern.DaysOfWeek[index] - result.DayOfWeek
                    : 7 - (int)result.DayOfWeek + (int)_pattern.DaysOfWeek[index]);
        }

        public override int GetIterationCount()
        {
            return _pattern.DaysOfWeek.Count;
        }
    }
}
